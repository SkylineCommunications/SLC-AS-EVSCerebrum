...
namespace CreateRoute_1
{
    ...
    public class Script
	{
        ...
        public void Run(Engine engine)
        {
            ...
            // Getting element
            var elementEVSCerebrum = engine.FindElementsByProtocol("EVS Cerebrum", "Production").FirstOrDefault();
            var dms = engine.GetDms();
            var evsElement = dms.GetElements().Where(x => x.Protocol.Name == "EVS Cerebrum" && x.Protocol.Version == "Production").FirstOrDefault();

            // Creating Route
            CreateRoute(engine, evsElement, device, source, destination);
        }
